...
#form a new departure column containing year, month, date, hour, minute
flights %>% 
...
#visualize dep_time
 # difference between geom_freqpoly and geom_histogram:
 # the former is a line, while the latter is chunks
 fl_dt %>% 
...
ymd(c("2010-10-10", "bananas")) #string can't parse
...
fl_dt %>% 
  # label= true, you get "Sunday"; false, "7"
  # abbr = false, you get "Sun"
...
fl_dt %>% 
  ...
  summarise(
    avg_delay = mean(arr_delay, 
                     na.rm= TRUE), # N?A values should be included
                     n=n()) %>%  ## there is a pipe, as here we didn't create new df, 
  # the order will will pass through ggplot to the end. It makes no difference to the graph
  #  n=n()) means a variable named n will be assigned the number of rows 
  # (think number of observations) in the summarized data.
ggplot(aes(x= minute, y= avg_delay)) +
    geom_line()

## what about the relation between sche_dep_time and arr_delay?
# this graph is basically thee same as last one, with the only difference being the variable
sched_dep <- fl_dt %>% 
  mutate(minute = minute(sched_dep_time)) %>% 
  group_by(minute) %>% 
  summarise(avg_delay = mean(arr_delay, na.rm = TRUE), n=n()) 
# here is no pipe, end of the order: we create a new df here
  ggplot(sched_dep, aes(x=minute, y=avg_delay)) + 
    ##to make ggplot work, you must add the data source in the 1st place
  geom_line()
...
fl_dt %>% 
    ...
    group_by(minute) %>% 
    summarise(avg_delay = mean(arr_delay, na.rm = TRUE), n=n()) %>%
  #here is a pipe, for ggplot no source need: it is fl_dt
  ggplot(aes(x=minute, y=avg_delay)) +
    geom_line() 
...
# another way: geom_bar will give you number of flights per week directly
fl_dt %>% 
  summarise(week = floor_date(dep_time, "week ")) %>% 
  group_by(week) %>% 
  ggplot(aes(week)) + 
  geom_bar() 

#show the distribution of flights across the course of the day 
# hint: you should convert all time slots into one day
fl_dt %>% 
  mutate(dep_hour= update(dep_time, yday=1 )) %>% 
  #no matter how yday changes, the graph is unchanged
  ggplot(aes(dep_hour)) +
