...
write.csv(data_raw, 'WDI_co2_raw.csv')
...
df <- read.csv("/Users/wodediannao/Desktop/ECBS-5208-Coding-1-Business-Analytics/Class_9/co2_assignment/WDI_co2_raw.csv")
...
write.csv(df,'WDI_co2_clean.csv')
...
# Call the data from github
df <- read.csv("/Users/wodediannao/Desktop/ECBS-5208-Coding-1-Business-Analytics/Class_9/co2_assignment/WDI_co2_clean.csv")
...
df <- df %>% mutate( gdptot = GDP_per*Population )
...
ggplot( df , aes(x = gdptot, y = CO2_emission)) +
  ...
  labs(x = "Total GDP (2015 int. const. $, PPP )",y = "CO2_emission  (in 2018)") 
