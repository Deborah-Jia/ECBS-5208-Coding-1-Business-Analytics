...
my [shanbay](https://www.wikihow.com/Forget-a-Bad-Memory#relatedwikihows)
...
```{r , include = FALSE}
library(tidyverse)
data_in <- "~/Desktop/ECBS-5208-Coding-1-Business-Analytics/Class_2/data/raw"
hv <- read.csv( paste0(data_in,"/hotelbookingdata.csv") )
```
...
```{r , echo = FALSE }
#echo is used to output results
summary(hv$price)
```
...
ggplot( data = hv , aes( x = price ) ) + 
  # within aes(), you can try density.count/ncount/ndensity
  geom_density( aes(y = ..density..) , fill = 'red' , bw = 15, alpha = 0.5 ) +
  labs(x='Hotel Prices in Vienna',y='Density') +
  scale_y_continuous(labels = scales::percent) # change the scale of y axis
```

## Different size of plots
```{r, echo=FALSE, out.width = '50%' }
# the difference between two graphs is the binwidth
ggplot( data = hv , aes( x = price ) ) +
